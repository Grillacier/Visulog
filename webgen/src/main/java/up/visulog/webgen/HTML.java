 ...
 */
	...
	public static String toHTML(AnalyzerResult analyzerResult){
	    HtmlView view = StaticHtml.view(v -> v
	        .html()
	          ...
	          .__() //head
	          .body()
	              .p().text(analyzerResult.getSubResults().stream().map(AnalyzerPlugin.Result::getResultAsHtmlDiv).reduce("", (acc, cur) -> acc + cur)).__()
	          .__() //body
	        .__()); //html et fin view
